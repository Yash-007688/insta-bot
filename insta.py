...
def download_from_url(api_client, url):
    ...
    try:
        ...
        if content_type == "stories":
            api_client.story_download(media_id, filename=str(out_dir))
            print(f"✓ Story downloaded to: {out_dir}")
        elif content_type == "reels":
            api_client.clip_download(media_id, filename=str(out_dir))
            print(f"✓ Reel downloaded to: {out_dir}")
        else:
            # Try photo first, fallback to general media download
            try:
                api_client.photo_download(media_id, filename=str(out_dir))
            except Exception:
                api_client.video_download(media_id, filename=str(out_dir))
            print(f"✓ Post downloaded to: {out_dir}")
